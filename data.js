...
async function fetchKlines(sym, tf, limit = 200) {
  ...
  // روش ۱: مستقیم (برای localhost یا اگه CORS باز شد)
  try {
    const r = await fetch(url, { signal: AbortSignal.timeout(5000) });
    if (r.ok) return (await r.json()).map(k => ({ t:+k[0],o:+k[1],h:+k[2],l:+k[3],c:+k[4],v:+k[5] }));
  } catch(e) {}

  // روش ۲: Cloudflare Worker proxy
  try {
    const r = await proxyFetch(url);
    return (await r.json()).map(k => ({ t:+k[0],o:+k[1],h:+k[2],l:+k[3],c:+k[4],v:+k[5] }));
  } catch(e) {
    if (typeof demoLog === 'function') demoLog(`⚠️ klines ${sym}: proxy هم کار نکرد — synth`);
  }

  // روش ۳: synth candles از قیمت WS
  const price = S.prices[sym]?.price || 0;
  if (price) return synthCandles(price, price * 1.02, price * 0.98, limit);
  throw new Error('fetchKlines failed: ' + sym);
}
...
async function fetch24h(sym) {
  ...
  try {
    ...
    let r;
    try { r = await fetch(url, { signal: AbortSignal.timeout(4000) }); }
    catch(e) { r = await proxyFetch(url); }
    if (!r?.ok) return;
...
function synthCandles(price, hi, lo, n = 120) {
  ...
  for (let i = 0; i < n; i++) {
    const o = p, h = o + Math.random() * range * .04, l = o - Math.random() * range * .04, cv = o + (Math.random() - .5) * range * .05;
    c.push({ o, h:Math.max(o,cv,h), l:Math.min(o,cv,l), c:cv, v:1000, t:Date.now() - ((n-i) * 300000) });
    p = cv;
  }
    ...
    if (isBinance) {
      ...
      try {
        ...
        if (e.message === 'AV_KEY_LIMIT') {
          ...
          const price = S.prices[sym]?.price || 0;
          if (price) S.candles[sym] = synthCandles(price, price * 1.02, price * 0.98);
        }
      }
    }
  } catch(e) {
